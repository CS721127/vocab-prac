...
/**
 * AiEnhancer — Google Gemma 3 API integration for vocabulary enhancement and
 * search.
 *
 * Use endpoint: POST
...
public class AiEnhancer {
    ...
    private static final String API_BASE_URL = "https://generativelanguage.googleapis.com/v1beta/models/gemma-3-27b-it:generateContent?key=";
    ...
    /**
     * Batch processes multiple words at once to reduce API calls and avoid 503 Rate
     * Limits.
    ...
    public static Map<String, String[]> enhanceVocabBatch(List<Vocab> batch, String apiKey) {
        if (apiKey == null || apiKey.isEmpty() || batch.isEmpty())
            return null;

        StringBuilder prompt = new StringBuilder();
        prompt.append(
                "You are a vocabulary assistant. I will provide a list of vocabulary words and their basic definitions.\n");
    ...
    private static String extractField(String text, String fieldName) {
        ...
        // If there's another known label indicating the end, use that (for multi-line
        // text)
        ...
        if (nextLabel1 != -1)
            minNext = Math.min(minNext, nextLabel1);
        if (nextLabel2 != -1)
            minNext = Math.min(minNext, nextLabel2);
        if (nextLabel3 != -1)
            minNext = Math.min(minNext, nextLabel3);
        if (nextLabel4 != -1)
            minNext = Math.min(minNext, nextLabel4);
    ...
    private static String callApi(String prompt, String apiKey) {
        try {
            URL url = URI.create(API_BASE_URL + apiKey).toURL();
